...
while True:
    ...
    if command=="": #exit program      
        ...
        entry=input("Enter polynomial coefficients: ")
        p=poly.get_polynomial(entry) # transform entry (str) into list of floats
        ...
        for i in range(len(list_poly)):
            print(str(i+1)+': Degree',poly.degree(list_poly[i]))
            
    elif command=="4": #evalute a given polynomial at different x values
        index=int(input("Which polynomial would you like to evaluate from [-5,5]? "))
        if not(1<=index<=len(list_poly)):
            continue
        for i in range(11):
            x=-5.0+i*1.0
